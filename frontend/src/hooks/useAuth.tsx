...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const login = async (
    ...
    try {
      ...
      if (axios.isAxiosError(error) && error.response) {
        ...
        // Проверяем, является ли ошибка связанной с верификацией email
        if (
  ...
  const register = async (
    ...
    try {
      ...
      // При регистрации НЕ логиним пользователя автоматически
      // Просто возвращаем успех, чтобы показать сообщение о верификации
      if (response.status === 200 || response.status === 201) {
        return { success: true };
      }
